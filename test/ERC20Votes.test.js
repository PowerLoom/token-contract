...
const MODES = {
  blocknumber: artifacts.require('PowerloomToken'),
};
...
contract('ERC20Votes', function (accounts) {
  ...
  const symbol = 'POWER';
  const version = '1';
  const supply = new BN('1000000000000000000000000000');

  for (const [mode, artifact] of Object.entries(MODES)) {
    describe(`vote with ${mode}`, function () {
      beforeEach(async function () {
        this.token = await artifact.new(holder);
      ...
      it('recent checkpoints', async function () {
        ...
        expect(await this.token.numCheckpoints(holder)).to.be.bignumber.equal('1');
        // recent
      ...
      describe('Compound test suite', function () {
        ...
        describe('balanceOf', function () {
          it('grants to initial account', async function () {
            expect(await this.token.balanceOf(holder)).to.be.bignumber.equal('1000000000000000000000000000');
          });
        });
